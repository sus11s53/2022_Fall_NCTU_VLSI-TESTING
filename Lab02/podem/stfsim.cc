...
void CIRCUIT::TFaultSimVectors()
{
    ...
    if (!Pattern.eof())
    { // For the following vectors, perform fault simulation
        while (!Pattern.eof())
        {
            ++pattern_num;
            if (pattern_num % 2 == 1)
...
void CIRCUIT::TFaultSim_t()
{
    ...
    for (fite = UTFlist.begin(); fite != UTFlist.end(); ++fite)
    {
        ...
        if (fptr->GetValue() == S0)
        { // Slow to fall; need a falling transition
        ...
        { // branch; Only inject faults at gate outputs,
            // so we need to propagate branch faults one level further
            if (!CheckTFaultyGate_t(fptr))
            {
                continue;
            }
...
void CIRCUIT::PrintIO_t()
{
    ...
    for (i = 0; i < No_PI(); ++i)
    {
        cout << PIGate(i)->GetValue_t();
    }
    cout << " ";
    for (i = 0; i < No_PO(); ++i)
    {
        cout << POGate(i)->GetValue_t();
    }
    cout << endl;
    return;
}
