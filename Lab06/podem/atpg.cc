...
{
    cout << "Generate stuck-at fault list for checkpoint Theorem" << endl;
    register unsigned i, j;
    GATEFUNC fun;
    GATEPTR gptr, fanout;
    FAULT *fptr;
...
void CIRCUIT::GenerateGoalFaultList()
{
    cout << "Generate stuck-at fault list" << endl;
    register unsigned i, j;
    GATEPTR gptr;
    ...
    {
        cout << "(" << (*it)->GetInputGate()->GetName() << "," << (*it)->GetOutputGate()->GetName() << "," << (*it)->GetValue() << ")" << endl;
    }
}
...
bool CIRCUIT::FaultEvaluate(FAULT *fptr)
{
    ...
    else if (ivalue == S1)
    {
        igptr->SetValue(D);
        // cout << "Implication" << endl;
        // cout << "set " << igptr->GetName() << " = D " << endl;
    }
    else if (ivalue == S0)
    {
        igptr->SetValue(B);
        // cout << "Implication" << endl;
        // cout << "set " << igptr->GetName() << " = B " << endl;
    }
    else
    {
        return false;
    ...
    // cout << "PI assignment" << endl;
    // cout << "set " << igptr->GetName() << " = " << igptr->GetValue() << endl;
    //  fault has propagated to output gate
    if (value != ogptr->GetValue())
    {
        ogptr->SetValue(value);
        // cout << "PO" << endl;
        // cout << "set " << ogptr->GetName() << " = " << ogptr->GetValue() << endl;
        return true;
    }
    return false;
}
...
GATEPTR CIRCUIT::TestPossible(FAULT *fptr)
{
    ...
    if (!ogptr->GetFlag(OUTPUT))
    {
        if (ogptr->GetValue() != X)
        {
            ...
            // search D-frontier
            // cout << "Fault propagation" << endl;
            decision_gptr = FindPropagateGate();
            // cout << "Select " << decision_gptr->GetName() << " to propagate" << endl;
            if (!decision_gptr)
            {
                return 0;
            }
...
GATEPTR CIRCUIT::FindPIAssignment(GATEPTR gptr, VALUE value)
{
    ...
    if (gptr->GetFunction() == G_PI)
    {
        gptr->SetValue(value);
        // cout << "PI assignment " << endl;
        // cout << "set " << gptr->GetName() << " = " << gptr->GetValue() << endl;
        return gptr;
    }
...
void CIRCUIT::RandomPattern()
{
    ...
    for (pattern_num = 0; pattern_num < 1000; pattern_num++)
    {
        ...
        fault_coverage = (detected_number) / double(total_number);

        if (fault_coverage > 0.9)
        {
            cout << "saturated fault coverage(90%)!";
            break;
        }
